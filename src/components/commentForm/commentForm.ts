...
export class CommentForm extends BaseComponent {
    ...
    setAvatar(setUserAvatar: BaseComponent, userAvatar: BaseComponent) {
    ...
    setName(inputValue: any, userName: BaseComponent) {
        userName.element.innerHTML = `<b>${(<HTMLInputElement>inputValue).value}</b>`;
    }

    render(review: ReviewData, setFormState: (state: boolean) => void) {
