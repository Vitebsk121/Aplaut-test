...
export class Author extends BaseComponent {
    ...
    setBeckgroundColor() {
        const colors = ['rgba(131, 182, 53, 0.5)', 'rgba(53, 182, 57, 0.5)', 'rgba(53, 133, 182, 0.5)', 'rgba(139, 53, 182, 0.5)'];
    ...
    render(review: ReviewData) {
        ...
        const postDate = new BaseComponent('p', ['post-info__date'], '' , `${new Date(Date.parse(review.published_at)).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' })}`);
        ...
        if(timeUsageNum !== null) {
            ...
            timeUsage = new BaseComponent('p', ['author-details__time-usage'], '', `Опыт использования ${timeUsageNum[numOfArrDetails].value}`);
        };
